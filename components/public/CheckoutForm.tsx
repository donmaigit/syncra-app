...
export default function CheckoutForm({ 
  ...
  providers, bankInfo, showAddress, styles 
}: CheckoutFormProps) {
  let t: any;
  ...
  // DEFAULTS FOR TYPOGRAPHY (Bold by default, overridden by sidebar)
  const titleStyle = { 
  ...
  const priceStyle = {
    ...
    fontSize: styles?.fontSize ? `calc(${styles.fontSize} * 1.25)` : '1.5rem',
      ...
      <div className="p-6 bg-slate-50 dark:bg-white/5 border-b border-slate-100 dark:border-white/5">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-slate-900 dark:text-white mb-1" style={titleStyle}>{productName}</h3>
            <p className="text-slate-900 dark:text-white" style={priceStyle}>¥{productPrice.toLocaleString()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6 text-left">
        {method === 'manual' && bankInfo && (
          <div className="p-5 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-900/30 text-sm">
             ...
             <div className="space-y-2 font-mono text-slate-700 dark:text-slate-300">
               <div className="grid grid-cols-[80px_1fr] gap-2 items-center"><span className="text-xs text-slate-500">{t('bank_info_labels.bank')}</span><span className="font-bold">{bankInfo.bankName || "---"}</span></div>
               <div className="grid grid-cols-[80px_1fr] gap-2 items-center"><span className="text-xs text-slate-500">{t('bank_info_labels.branch')}</span><span className="font-bold">{bankInfo.branchName || "---"}</span></div>
               <div className="grid grid-cols-[80px_1fr] gap-2 items-center"><span className="text-xs text-slate-500">{t('bank_info_labels.number')}</span><span className="font-bold">{bankInfo.accountNumber || "---"}</span></div>
               <div className="grid grid-cols-[80px_1fr] gap-2 items-center"><span className="text-xs text-slate-500">{t('bank_info_labels.holder')}</span><span className="font-bold">{bankInfo.accountHolder || "---"}</span></div>
             </div>
          </div>
        )}
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 gap-3">
            <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('customer_name')}</label><input type="text" value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder={t('name_placeholder')} className="w-full p-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-lg text-sm outline-none focus:ring-2 focus:ring-purple-500" /></div>
            <div><label className="block text-xs font-bold uppercase text-slate-500 mb-1">{t('customer_email')}</label><input type="email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} placeholder="you@example.com" className="w-full p-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-lg text-sm outline-none focus:ring-2 focus:ring-purple-500" /></div>
          </div>
          {showAddress && (
        ...
        <button onClick={handleBuy} disabled={loading} className={`w-full py-4 text-white rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all ${method === 'card' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'}`} style={buttonStyle}>
        ...
        <p className="text-center text-xs text-slate-400 flex items-center justify-center gap-1"><ShieldCheck size={12}/> {method === 'card' ? t('secure_checkout') : t('bank_secure_note')}</p>
      </div>
    </div>
  );
}